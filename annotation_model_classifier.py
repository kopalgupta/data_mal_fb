###### using human annotations (technique 2)
...
# df = data frame, 0.2 = 80-20 division, unbiased and random splitting
...
# features
# M_app and Z_class are 2 non-numerical features
...
#Feature engineering
def convert_to_bool(x): # function for coverting non-numerical feature to numerical
	return 1 if x == 'Spam' else 0

labels = training_partition['Z_CLASS'].apply(convert_to_bool) # coverting non-numerical feature z_class to numerical



app_to_num_lookup = dict(mobile=0, web=1, thirdparty=2, contentsharing=3, other=4) # coverting non-numerical feature M_app to numerical
...
def app_to_num(x): # assigning numerical codes
	return app_to_num_lookup[x]
...
result = alg.predict(test_set) # predict on the test set that was 20% of the orifinal dataset

#Results of the prediction
print "Result: ",result

#Predicting the values from the test set
print alg.predict_proba(test_set) # predicting the probability of the prediction
# With how much probability or confidence if algo has been able to predict whether it is spam or not

#The order of classes
#0 -> NotSpam		1 -> Spam
print "classes: ",alg.classes_


#Get accuracu score
from sklearn.metrics import accuracy_score
print "Accuracy score: ",accuracy_score(true_labels,result) # here we pass the true labels and the predictions by the algo
